 ...
 */
    ...
    $current_year = date('Y');
    $programmed_year = date("Y", $event->date_string);  
?>

<?php if($current_year === $programmed_year){?>
...
<div class="flex-row flex-between event-container flex-top">
        ...
        <div class="event-details flex-column">
            ...
            <div class="flex-column flex-top"><?php

                ?><div class="flex-row fullwidth change-to-column-on-mini-phone">
                    <div class="flex-row">
                        <?php if($short_date){ ?>
                            <p class="event-detail date"><i><?php echo $short_date?></i></p><?php
                        }
                        if($event->date_string){?>
                            <p class="event-detail startdate unbreak"><u class="dot">•</u><i><?php echo date("H:i", $event->date_string); ?>-<?php echo date("H:i", strtotime($event->end_date[0])); ?> <?php echo $event-> timezone[0]?></i></p><?php
                        } ?>
                    </div>
        ...
        <?php if ( !empty( get_the_content() ) ){
            ?>
            <svg class="event-expander primary-expander" width="65" height="65" viewBox="0 0 65 65" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M32.4688 64.3125C49.9062 64.3125 64.3438 49.875 64.3438 32.4375C64.3438 15.0312 49.875 0.5625 32.4375 0.5625C15.0312 0.5625 0.59375 15.0312 0.59375 32.4375C0.59375 49.875 15.0625 64.3125 32.4688 64.3125ZM35.7188 41.9688C33.6875 44.125 31.25 44.125 29.2188 41.9688L16.3438 28.2812C15.3438 27.2188 15.3438 25.75 16.25 24.8125C17.25 23.75 18.8125 23.7188 19.7812 24.75L32.4688 38.1875L45.1875 24.75C46.125 23.7188 47.6875 23.75 48.6875 24.8125C49.5938 25.75 49.5938 27.2188 48.5938 28.2812L35.7188 41.9688Z" fill="currentColor"/>
            </svg>
